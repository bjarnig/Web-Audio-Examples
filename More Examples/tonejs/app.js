...
function toneJS3() {
  ...
  // set the transport to repeat at 2 measures
...
function toneJS5() {
  ...
  const feedbackDelay = new Tone.FeedbackDelay("8n", 0.8).toMaster();

  // Create a synth and 'chain' the effects, then play a chord
  const polySynth = new Tone.PolySynth(4, Tone.Synth).chain(
    distortion,
    tremolo,
    feedbackDelay,
    Tone.Master
...
function toneJS6() {
  const reverb = new Tone.Reverb({ 'decay' : 10 }).toMaster();
  const plucky = new Tone.PluckSynth().toMaster();

  reverb.generate().then(() => {
    plucky.connect(reverb);
    plucky.triggerAttack('C' + Math.random() * 10);
  });
}
